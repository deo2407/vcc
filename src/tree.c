...
// Builds the root tree, stores it and returns its hash
int build_tree(char *hash, size_t max_size) {
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        sprintf(entry_str, "%s blob %s", mode_to_string(MODE_BLOB_REG), line);
        strcat(content, entry_str);
    }

    add_tree(content, tree_body_len + 1);
    get_hash(content, tree_body_len + 1, hash, max_size);
